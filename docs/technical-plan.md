...
请求固定使用一条 `system` 消息和一条 `user` 消息。确定性处理后的识别结果原样作为 `user` 消息，不增加包装指令。用户配置的基础系统提示词由程序自动追加当前用户词典快照；词典不依赖提示词占位符，因此用户修改提示词不会意外关闭词典注入。每次会话在进入润色前取得一次 Provider、提示词与词典快照，配置热更新只影响后续会话。
