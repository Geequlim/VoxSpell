...
- 润色过程中始终展示两个候选：一号为实时润色结果，二号为最终识别结果。
- 润色期间顶部保持 `polishing` 动画；按 `2` 可以立即放弃润色并提交识别结果，不进入可见的“提交中”状态。
- 一号润色结果完成前不可选择；润色完成后，只有关闭自动提交时才进入 `choosing`。
- 如果最终润色文本与识别文本完全一致，客户端会直接提交识别结果，跳过没有实际差异的候选选择。
...
- 识别结果固定保留为第二候选，允许用户绕过 AI 润色；该行为与自动提交模式一致。
